<?php
// --- 1. SETTING ERROR AGAR MUNCUL (DEBUGGING) ---
...
// --- 2. FUNGSI FORMAT RUPIAH CADANGAN (JAGA-JAGA) ---
...
<?php if($orders->num_rows > 0): ?>
        <?php while($order = $orders->fetch_assoc()): 
            ...
            // --- PERBAIKAN UTAMA DI SINI (ANTI ERROR) ---
            // Menggunakan tanda '??' untuk mencegah error jika kolom tidak ditemukan
            
            // 1. Ambil data status dengan aman
            $raw_status = $order['status'] ?? 'pending'; 
            // 2. Ambil data pembayaran dengan aman
            $raw_payment = $order['payment_status'] ?? 'unpaid'; 
            // 3. Ambil alamat dengan aman
            $raw_address = $order['shipping_address'] ?? '-';
            // 4. Ambil notes dengan aman
            $raw_notes   = $order['notes'] ?? '-';

            // --- LOGIKA BADGE ---
            $current_status = isset($status_badges[$raw_status]) 
                ? $status_badges[$raw_status] 
                : ['class' => 'badge-secondary', 'text' => strtoupper($raw_status)];

            $current_payment = isset($payment_badges[$raw_payment]) 
                ? $payment_badges[$raw_payment] 
                : ['class' => 'badge-secondary', 'text' => strtoupper($raw_payment)];
        ?>
        <div class="order-card">
            <div class="order-header">
                <div>
                    <div class="order-number"><?php echo $order['order_number'] ?? 'INV-???'; ?></div>
                    <div class="order-date">
                        <i class="fas fa-calendar"></i> 
                        <?php 
                            $date = $order['order_date'] ?? date('Y-m-d H:i:s');
                            echo date('d F Y, H:i', strtotime($date)); 
                        ?>
                    </div>
                </div>
                <div class="order-badges">
                    ...
                    <span class="badge <?php echo $current_payment['class']; ?>">
                        <?php echo $current_payment['text']; ?>
                    </span>
                </div>
            </div>
            ...
            <div class="order-body">
                <div class="order-info">
                    <div class="info-item">
                        ...
                        <div class="info-value"><?php echo substr($raw_address, 0, 50); ?>...</div>
                    </div>
                    
                    <div class="info-item">
                        <div class="info-label">Catatan</div>
                        <div class="info-value"><?php echo $raw_notes; ?></div>
                    </div>
                    
                    <div class="info-item">
                        <div class="info-label">Total Pembayaran</div>
                        <div class="order-total"><?php echo formatRupiah($order['total_amount'] ?? 0); ?></div>
                    </div>
                </div>
    ...
    function viewDetails(orderId) {
        fetch('get_order_details.php?order_id=' + orderId)
            ...
            .then(data => {
                            ...
                            <div class="detail-item">
                                <div>
                                    ...
                                    <span style="color: #7f8c8d; font-size: 14px;">
                                        ${item.quantity} x ${formatRupiahJS(item.price)}
                                    </span>
                                </div>
                                <strong style="color: #27ae60;">${formatRupiahJS(item.subtotal)}</strong>
                            </div>
                        `;
                    });
                }
                ...
                else if (data.order && data.order.total_amount) total = data.order.total_amount;
                
                html += `<span style="color: #27ae60;">${formatRupiahJS(total)}</span>`;
                html += '</div></div>';
            ...
            .catch(err => {
                console.error(err);
                document.getElementById('modalBody').innerHTML = "<p style='color:red'>Gagal memuat detail.</p>";
            });
    }
